...
exports.calculateAuthoShotgunDmg = ({ logStr, shooterObj, targetObj, shooterAimMods, battleData}) => {
    for(let i = 0; i < battleData.wpnBullets; i++) {
        ...
        //reduce every shot aim with -2 mod for single shot mod when using high ROF weapons
        accumulatedAim = accumulatedAim - (2 * shotNumber);
        if(accumulatedAim >= battleData.shotComplexity) {
            ...
            if(shotNumber > 0){
                logStr = `${logStr}<div class="shot-landed">Aim value was decreased by <span class="shot-value">${2 * shotNumber}</span> as a debuff for consecutive action!</div>`
            }
